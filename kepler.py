...
def get_dydx(x, y, dx):
    ...
    x_vec = 2 * np.arange(nbodies)
    y_vec = 2 * np.arange(nbodies) + 1
    v_x = 2 * np.arange(nbodies) + 2 * nbodies
    v_y = 2 * np.arange(nbodies) + 2 * nbodies + 1
    ...
    for i in range(nbodies):
        ...
        for j in range(nbodies):
            if (i != j):
                xs = y[x_vec[i]] - y[x_vec[j]]
                ys = y[y_vec[i]] - y[y_vec[j]]
                r_ij = ((xs**2) + (ys**2))**0.5
                s_x += masses[j] * ((xs)/(r_ij**3))
                s_y += masses[j] * ((ys)/(r_ij**3))
        dydx[v_x[j]] = -gnewton * s_x
        dydx[v_y[j]] = -gnewton * s_y
...
def ode_check(x,y,it):
    ...
    ymin    = np.min(y[indy,:])
    ymax    = np.max(y[indy,:])
    fig1    = plt.figure(num=1,figsize=(6,6),dpi=100,facecolor='white')
    ...
    fig2    = plt.figure(num=2,dpi=100,facecolor='white')
    ax2     = fig2.add_subplot(211)
    ax2.plot(x,Eplot,linestyle='-',color='black',linewidth=1.0,label='E')
    ax2.plot(x,Lplot,linestyle='--',color='black',linewidth=1.0,label='L')
    ax2.set_xlabel('t [yr]')
    ax2.set_ylabel('E/E0-1, L/L0-1')
    ax2.legend()
    ax3     = fig2.add_subplot(212)
    ax3.plot(x,logE,linestyle='-',color='black',linewidth=1.0,label='E')
    ax3.plot(x,logL,linestyle='--',color='black',linewidth=1.0,label='L')
    ax3.set_xlabel('t [yr]')
    ax3.set_ylabel('log|E/E0-1|, log|L/L0-1|')
    ax3.legend()
