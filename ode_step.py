...
def euler(fRHS,x0,y0,dx):
    ...
    # dx ~ the average slope within the interval y0'
    y  = y0 + dx*fRHS(x0,y0,dx)
    
    #??????????????????????????????????????????????????????????
    
    return y,1 # a (y0 x 1) size vector?
...
def rk2(fRHS,x0,y0,dx):
    ...
    # the MIDPOINT METHOD gives a = 0, b = 1, alpha = 1/2, and beta = 1/2
    k1 = dx * fRHS(x0, y0, dx)
...
def backeuler(fRHS,x0,y0,dx):
    ...
    C = np.array([[998.0,1998.0],[-999.0,-1999.0]])
    I = np.identity(2)
    #y = np.dot(np.linalg.inv(I - C*dx), y0)
    y = y0 + (dx * np.dot(np.linalg.inv(I - dx*C), fRHS(x0, y0, dx)))
